<?php
/**
 * Helper function to create chart SVG.
 */

function besan_get_svg( $data ) {
  $consts = _besan_set_consts( $data );
  $svg = '';

  _besan_get_svg_start( $svg, $consts );
  _besan_get_data( $svg, $data, $consts );
  _besan_get_svg_end( $svg );
...
function _besan_set_consts( $data ) {
  $consts = array(
    'height_total_px'  => 400,
    'height_chart_px'  => 360,
    'label_x_axis_px'  => 380,
...
function _besan_get_svg_start( &$svg, $consts ) {
  ...
  $svg .= '<line x1="' . $consts['offset_chart_pct'] . '%" y1="0" x2="10%" y2="' . $consts['height_chart_px'] . '" stroke="' . $consts['base_color'] . '" stroke-width="2" />';
  ...
  $svg .= '<text x="5%" y="10" fill="' . $consts['base_color'] . '" font-size="' . $consts['base_font_size'] . '">' . $consts['max_value'] . '</text>';
  $svg .= '<text x="5%" y="' . $consts['height_chart_px'] . '" fill="' . $consts['base_color'] . '" font-size="' . $consts['base_font_size'] . '">0</text>';
  $svg .= '</g>';
}


// [HELPER] Function to write out the bars of data.
function _besan_get_data( &$svg, $data, $consts ) {
  ...
  foreach ( $data as $key => $d ) {
    ...
    $svg .= '<text x="' . $offset . '%" y="' . $consts['label_x_axis_px'] . '" fill="' . $consts['base_color'] . '" font-size="' . $consts['base_font_size'] . '">' . $key . '</text>';
    $svg .= '</g>';
...
function _besan_get_svg_end( &$svg ) {
  $svg .= '</svg>';
}
